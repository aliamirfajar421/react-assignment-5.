...
export const useFoodStore = create((set, get) => ({
  ...
  isOrderVisible: true, // 👈 1. ٹریکر ونڈو کو اوپن/منیمائز رکھنے کی سٹیٹ

  toggleCart: () => set((state) => ({ isCartOpen: !state.isCartOpen })),
  toggleSpin: () => set((state) => ({ isSpinOpen: !state.isSpinOpen })),
  setCategory: (category) => set({ activeCategory: category }),
  setSearchQuery: (query) => set({ searchQuery: query }),
  setOrderVisible: (visible) => set({ isOrderVisible: visible }), // 👈 2. ٹریکر کو منیمائز/اوپن کرنے کا فنکشن
